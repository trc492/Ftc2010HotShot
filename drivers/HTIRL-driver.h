/*
 * $Id: HTIRL-driver.h 20 2009-12-08 22:59:13Z xander $
 */
 ...
 * @param data the data the header should be added to
 */
inline void addI2CHead(tByteArray &data) {
 ...
 *
 * Note: this is an internal function and should not be called directly.
 * @param data the data the tail should be appended to
 */
inline void addI2CTail(tByteArray &data) {
  data.arr[BUF_HEADSIZE + BUF_DATASIZE] = 11;         // Total IR command length
  data.arr[BUF_HEADSIZE + BUF_DATASIZE + 1] = 0x02;   // IRLink mode 0x02 is PF motor
